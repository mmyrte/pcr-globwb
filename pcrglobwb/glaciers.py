...
def updateStaticGlacier(self, meteo, currTimeStep):
    ...
    if currTimeStep.doy == 244:
        ...
        self.glacierAccumulation = pcr.ifthenelse(
            self.snowCoverSWE > self.acclim, self.snowCoverSWE - self.acclim, 0
        )
...
def updateDeltaH(self, currTimeStep):
    ...
    glacierDiff = pcr.ifthenelse(
        pcr.pcrand(pcr.ordinal(frac) != 100, pcr.abs(glacierGap) > 1e-3),
    ...
    positiveGap = pcr.ifthenelse(
        pcr.pcror((frac < 0), pcr.pcrand((frac == 0), (glacierGap > 0))),
        glacierDiff,
        0.0,
    )
    glacierDiff = pcr.ifthenelse(
        pcr.pcror((frac < 0), pcr.pcrand((frac == 0), (glacierGap > 0))), 0, glacierDiff
    )
