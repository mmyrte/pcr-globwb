...
class LandCover(object):
    ...
    def __init__(
        ...
        if "snowTransport" in list(self.iniItemsLC.keys()):
            if self.iniItemsLC["snowTransport"] == "False":
        ...
        if "glacierModule" in list(iniItems.landSurfaceOptions.keys()):
            if iniItems.landSurfaceOptions["glacierModule"] == "True":
                self.glacierModule = True
            elif iniItems.landSurfaceOptions["glacierModule"] == "False":
        ...
        if "albedoMelt" in list(self.iniItemsLC.keys()):
            if self.iniItemsLC["albedoMelt"] == "True":
                self.albedoMelt = True
            elif self.iniItemsLC["albedoMelt"] == "False":
                self.albedoMelt = False
        else:
            self.albedoMelt = False

        if "seasonalMelt" in list(self.iniItemsLC.keys()):
            if self.iniItemsLC["seasonalMelt"] == "True":
                self.seasonalMelt = True
            elif self.iniItemsLC["seasonalMelt"] == "False":
        ...
        if "precipitationMelt" in list(self.iniItemsLC.keys()):
            if self.iniItemsLC["precipitationMelt"] == "True":
                self.precipitationMelt = True
            elif self.iniItemsLC["precipitationMelt"] == "False":
        ...
        if self.albedoMelt:
            ...
            vars(self)["degreeDayRange"] = vos.readPCRmapClone(
                input, self.cloneMap, self.tmpDir, self.inputDir
            )
            vars(self)["degreeDayRange"] = pcr.spatial(
                pcr.scalar(vars(self)["degreeDayRange"])
            )
        ...
        if self.seasonalMelt:
            ...
            vars(self)["degreeDayAmplitude"] = vos.readPCRmapClone(
                input, self.cloneMap, self.tmpDir, self.inputDir
            )
            vars(self)["degreeDayAmplitude"] = pcr.spatial(
                pcr.scalar(vars(self)["degreeDayAmplitude"])
            )
        ...
        if self.snowTransport != False:
            ...
            self.gradient = vos.readPCRmapClone(
                iniItems.routingOptions[str("gradient")],
                self.cloneMap,
                self.tmpDir,
                self.inputDir,
            )
            ...
            self.verticalSizeInMeter = cellSizeInArcMin * 1852.0
            ...
            # Be smart: add these kind of things to the landsurface object!!!!!!!
            if (self.snowTransport == "FreyAndHolzmann") or (
                self.snowTransport == "FreyAndHolzmann_pcraster"
            ):
                logger.info("Initialising Frey and Holzmann...")
                input = self.iniItemsLC["Hv"]
                vars(self)["Hv"] = vos.readPCRmapClone(
                    input, self.cloneMap, self.tmpDir, self.inputDir
                )
                vars(self)["Hv"] = pcr.spatial(pcr.scalar(vars(self)["Hv"]))

                input = self.iniItemsLC["frho"]
                vars(self)["frho"] = vos.readPCRmapClone(
                    input, self.cloneMap, self.tmpDir, self.inputDir
                )
    ...
    def updateLC(
        ...
        # %% ADDED BY JOREN: START
        elif self.snowModuleType == "Alpine":
            self.snowMeltSlaterAndClark(meteo, currTimeStep)
        # %% ADDED BY JOREN: STOP
    ...
    def interceptionUpdate(self, meteo, currTimeStep):
        ...
        solid_fraction = pcr.ifthenelse(
            (meteo.temperature >= self.freezingT) & (meteo.temperature < self.liquidT),
            1 - meteo.temperature / (self.liquidT - self.freezingT),
            0.0,
        )
        estimSnowfall = estimSnowfall + solid_fraction * meteo.precipitation
        # ADDED BY JOREN: STOP
        ...
        if self.snowfallCorrection != False:
            logger.info("Correcting snowfall!!!")
            self.snowfall = self.snowfall * self.snowfallCorrection
    ...
    def snowMeltSlaterAndClark(self, meteo, currTimeStep):
        ...
        if self.snowTransport == "FreyAndHolzmann":
            if (currTimeStep.day == 1) or (currTimeStep.day == 15):
                # Only Run Monthly?
                self.simplifiedFreyAndHolzmann()
        elif self.snowTransport == "FreyAndHolzmann_pcraster":
            # Only Run Monthly?
            self.simplifiedFreyAndHolzmann_pcraster()

        elif self.snowTransport == "Basic":
            if (currTimeStep.day == 1) or (currTimeStep.day == 15):
                # Only Run Monthly?
                self.critSWE = self.maxSWE(
                    self.angle, Smin=self.Smin, SS1=self.SS1, SS2=self.SS2
                )
        ...
        if self.albedoMelt == True:
            ...
            self.PDD = pcr.ifthenelse(
                pcr.pcrand(self.snowfall < 0.005, self.snowCoverSWE > 0.0),
                self.PDD + pcr.max(meteo.temperature_max, 0),
                0.001,
            )
            self.albedo = 0.713 - 0.112 * pcr.log10(self.PDD)
            self.albedo = pcr.max(self.albedo, 0.4)
            self.albedo = pcr.min(self.albedo, 0.85)
            self.degreeDayFactor = (self.DDFmax - self.DDFmin) / (
                1 / 0.4 - 1 / 0.85
            ) * (1 / self.albedo - 1 / 0.85) + self.DDFmin
        else:
            ...
            deltaSnowCover = pcr.ifthenelse(
                ...
                -pcr.min(
                    ...
                    * pcr.max(
                        ...
                        + self.degreeDayAmplitude
                        * np.sin((currTimeStep.doy - 81) * 2 * np.pi / 366),
                        0.0,
                    ),
                ),
            )
        ...
        if self.debugWaterBalance:
            ...
            if (
                self.snowTransport == "Complete"
                or self.snowTransport == "Basic"
                or self.snowTransport == "FreyAndHolzmann"
                or self.snowTransport == "FreyAndHolzmann_pcraster"
            ):
                logger.info(
                    "WBCHECK including SnowTransport: " + str(self.snowTransport)
                )
            else:
                logger.info(
                    "WBCHECK including SnowTransport: " + str(self.snowTransport)
            ...
            vos.waterBalanceCheck(
                [self.liquidPrecip - self.rain2GlacierWater, -1 * deltaSnowCover],
                [
                    self.actSnowFreeWaterEvap,
                    self.netLqWaterToSoil - self.netGlacierWaterToSoil,
                    self.snow2GlacierWater,
            ...
            vos.waterBalanceCheck(
                [
                    self.rain2GlacierWater,
                    self.snow2GlacierWater,
                    self.glacierAccumulation,
                    self.capacity2GlacierWater,
                ],
    ...
    def snowSlideBasic(self, currTimeStep):
        ...
        self.transportVolSnow = pcr.max(
            (self.snowCoverSWE - critSWE) * self.cellArea, 0.0
        )

        # Convert everything to volumes
        snowCoverSWEVol = pcr.accuthresholdstate(
            self.lddMap, self.snowCoverSWE * self.cellArea, critSWE * self.cellArea
        )

        self.incomingVolSnow = pcr.max(
            snowCoverSWEVol - self.snowCoverSWE * self.cellArea, 0.0
        )

        self.snowCoverSWE = snowCoverSWEVol / self.cellArea
        logger.info("Finished with SnowSlide BASIC! Not so impressive..")

    def simplifiedFreyAndHolzmann(self):
        logger.info(
            "Starting with Frey and Holzmann BASIC: let's see how far we get...."
        )
        ...
        snowCoverSWEVol = pcr.accufractionstate(
            self.lddMap, transportVolSnow, pcr.min(self.angle / 90 * self.frho, 1.0)
        )

        self.transportVolSnow = (
            transportVolSnow  # *pcr.min(self.angle/90*self.frho, 1.0)
        )
        self.incomingVolSnow = snowCoverSWEVol

        self.snowCoverSWE = (
            self.snowCoverSWE - exceedingSnow + self.incomingVolSnow / self.cellArea
        )

        logger.info("Finished with Frey and Holzmann BASIC! Not so impressive..")

    def simplifiedFreyAndHolzmann_pcraster(self):
        logger.info(
            "Starting with Frey and Holzmann PCRASTER: let's see how far we get...."
        )
        ...
        transportVolSnow = pcr.max(exceedingSnow * self.cellArea, 0.0)
        # Convert everything to volumes
        # snowCoverSWEVol = pcr.accufractionstate(self.lddMap, transportVolSnow, pcr.min(self.angle/90*self.frho, 1.0))
        ...
        snowCoverSWEVol = pcr.downstream(self.reverseLDD, fractionTransport)
    ...
    def snowSlide(self, currTimeStep):
        ...
        yslope = abs(
            (elevation[2:, :] - elevation[:-2, :]) / (2 * self.verticalSizeInMeter)
        )
        y_lower = abs((elevation[1, :] - elevation[0, :]) / (self.verticalSizeInMeter))
        y_upper = abs(
            (elevation[-1, :] - elevation[-2, :]) / (self.verticalSizeInMeter)
        )
        yslope = np.vstack((y_lower, yslope, y_upper))

        dx = zonalDistance[:, 0:1]
        xslope = abs((elevation[:, 2:] - elevation[:, :-2]) / (2 * dx))
        x_left = np.expand_dims(
            abs((elevation[:, 1] - elevation[:, 0]) / (dx[:, 0])), axis=1
        )
        x_right = np.expand_dims(
            abs((elevation[:, -1] - elevation[:, -2]) / (dx[:, -1])), axis=1
        )
        xslope = np.hstack((x_left, xslope, x_right))
        slope = np.sqrt(yslope**2 + xslope**2)
        angle = np.rad2deg(np.arctan(slope))
        ...
        while (np.sum(transportVolSnow) > 0) & (loops < 5):
            ...
            for height in np.sort(np.unique(relevant_dem.flatten()))[::-1][:-1]:
                ...
                for i in range(len(locs[0])):
                    ...
                    elevdiff = (
                        elevation[ymin:ymax, xmin:xmax] - elevation[ycenter, xcenter]
                    )
                    elevdiff[elevdiff > 0] = 0
                    reldiff = elevdiff / np.sum(elevdiff)

                    # Add the incoming SWE
                    incomingVolSnow[ymin:ymax, xmin:xmax] += (
                        reldiff * transportVolSnow[ycenter, xcenter]
                    )
        ...
        self.snowCoverSWE = pcr.numpy2pcr(
            pcr.Scalar, np.asarray(snowCoverSWE.tolist()), np.nan
        )
        self.transportVolSnow = pcr.numpy2pcr(
            pcr.Scalar, np.asarray(transportVolSnowTotal.tolist()), np.nan
        )
        self.incomingVolSnow = pcr.numpy2pcr(
            pcr.Scalar, np.asarray(incomingVolSnowTotal.tolist()), np.nan
        )

        # INPUT IS DAN WEER ANDERS?

        logger.info(
            "Finished with SnowSlide! Impressive.. Number of loops:" + str(loops)
        )

    def maxSWE(self, S, Smin=25, SS1=250, SS2=0.172):
        SWE = pcr.ifthenelse(S < Smin, 5, SS1 * pcr.exp(-SS2 * (S)))
        SWE = pcr.max(SWE, 0.05)
        return SWE
    ...
    def updateSoilStates(self):
        ...
        if self.glacierModule == False:
            ...
            self.landSurfaceRunoff = (
                self.directRunoff + self.interflowTotal + self.glacierOutflow
            )
