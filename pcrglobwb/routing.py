...
class Routing(object):
    ...
    def update(self, landSurface, groundwater, currTimeStep, meteo):
        ...
        self.dynamicFracWat_excluding_flooding += pcr.max(
            0.0, 1.0 - self.dynamicFracWat_excluding_flooding
        ) * pcr.max(self.channelFraction, 0.0)
        # - fraction of lake and reservoir, as well as channel, but excluding flood
        self.dynamicFracWat_excluding_flooding = pcr.ifthen(
            self.landmask, pcr.min(1.0, self.dynamicFracWat_excluding_flooding)
        )
    ...
    def kinematic_wave_update_before_19_feb_2018(
        self, landSurface, groundwater, currTimeStep, meteo
    ):
    ...
    def kinematic_wave_update_not_working(
        self, landSurface, groundwater, currTimeStep, meteo
    ):
        ...
        for i_loop in range(number_of_loops):
            ...
            channelStorageForRouting = pcr.max(
                0.0, storageAtLakeAndReservoirs - channelStorageForRouting
            )
            #
            # update waterBodyStorage (inflow, storage and outflow)
            ...
            # - update channelStorage (m3)  - after waterBodyOutflow (m3)
            storage_change_in_volume = (
                pcr.upstream(self.lddMap, waterBodyOutflow) - waterBodyOutflow
            )
            channelStorageForRouting += storage_change_in_volume
