...
class SoilAndTopoParameters(object):
    ...
    def readSoilMapOfFAO(self, iniItems, optionDict=None):
        ...
        if "calibrationK" in list(iniItems.landSurfaceOptions.keys()):
            if iniItems.landSurfaceOptions["calibrationK"] == "True":
                self.calibrationK = True
            elif iniItems.landSurfaceOptions["calibrationK"] == "False":
        ...
        if self.calibrationK == True:
            if "calibrationKwhich" in list(iniItems.landSurfaceOptions.keys()):
                if iniItems.landSurfaceOptions["calibrationKwhich"] == "upper":
                    self.calibrationKwhich = "upper"
                elif iniItems.landSurfaceOptions["calibrationKwhich"] == "lower":
                    self.calibrationKwhich = "lower"
            else:
                self.calibrationKwhich = "both"

            print(
                "----WARNING: CHANGING K VALUES!!--------------------------------------------------------"
            )
    ...
    def readSoil(self, iniItems, optionDict=None):
        ...
        #
        if self.numberOfLayers == 2:
            self.interflowConcTime = (
                (self.kSatLow * self.tanslope * 2.0)
                / (
                    self.slopeLength
