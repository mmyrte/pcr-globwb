...
class LandCover(object):
    def __init__(
        ...
        if "preferentialFlow" in list(self.iniItemsLC.keys()):
            if self.iniItemsLC["preferentialFlow"] == "True":
                self.preferentialFlow = True
            elif self.iniItemsLC["preferentialFlow"] == "False":
                self.preferentialFlow = False
        else:
            self.preferentialFlow = False
        if self.preferentialFlow == True:
            self.forestFrac = vos.readPCRmapClone(
                self.iniItemsLC["forest"], self.cloneMap, self.tmpDir, self.inputDir
            )
        ...
        #     if (self.snowTransport=='FreyAndHolzmann') or (self.snowTransport=='FreyAndHolzmann_pcraster') :
    ...
    def calculateTotAvlWaterCapacityInRootZone(self):
        ...
        if self.numberOfLayers == 2:
            self.totAvlWater = (
                ...
                * (
                    self.parameters.satVolMoistContUpp
                    - self.parameters.resVolMoistContUpp
                )
                ...
                * (
                    self.parameters.satVolMoistContLow
                    - self.parameters.resVolMoistContLow
                )
                * pcr.min(
    ...
    def updateLC(
        ...
        # Run the new snow module.
            ...
            sn.snowMeltSlaterAndClark(self, meteo, currTimeStep)
        # %% ADDED BY JOREN: STOP
    ...
    def interceptionUpdate(self, meteo, currTimeStep):
        ...
        # solid_fraction=pcr.ifthenelse((meteo.temperature>=self.freezingT)&(meteo.temperature<self.liquidT), 1-meteo.temperature/(self.liquidT-self.freezingT), 0.0)
    ...
    #         if (currTimeStep.day==1) or (currTimeStep.day==15):
    #             #Only Run Monthly?
    #             self.critSWE=self.maxSWE(self.angle, Smin=self.Smin, SS1=self.SS1, SS2=self.SS2)
    ...
    #     logger.info('Starting with Frey and Holzmann BASIC: let\'s see how far we get....')
    ...
    #     self.transportVolSnow=transportVolSnow#*pcr.min(self.angle/90*self.frho, 1.0)
    #     self.incomingVolSnow= snowCoverSWEVol

    #     self.snowCoverSWE = self.snowCoverSWE-exceedingSnow+self.incomingVolSnow/self.cellArea
    ...
    def getSoilStates(self):
        ...
        if self.numberOfLayers == 2:
            ...
            #
            # ADDED BY JOREN: START
            if self.preferentialFlow == False:
                # ADDED BY JOREN: STOP
            ...
            # ADDED BY JOREN: START
                ...
                modifiedKSatUpp = pcr.ifthenelse(
                    self.effSatUpp > self.S0Upp,
                    self.parameters.kSatUpp
                    + self.parameters.kSatUpp
                    * fm
                    * (self.effSatUpp - self.S0Upp)
                ...
                modifiedKSatLow = pcr.ifthenelse(
                    self.effSatLow > self.S0Low,
                    self.parameters.kSatLow
                    + self.parameters.kSatLow
                    * fm
                    * (self.effSatLow - self.S0Low)
                ...
                self.kUnsatUpp = pcr.max(
                    0.0,
                    (self.effSatUpp**self.parameters.campbellBetaUpp) * modifiedKSatUpp,
                )  # original Rens's code: KTHEFF1= max(0,THEFF1**BCB1[TYPE]*KS1[TYPE])
                self.kUnsatLow = pcr.max(
                    0.0,
                    (self.effSatLow**self.parameters.campbellBetaLow) * modifiedKSatLow,
                )  # original Rens's code: KTHEFF2= max(0,THEFF2**BCB2[TYPE]*KS2[TYPE])
                self.kUnsatUpp = pcr.min(self.kUnsatUpp, modifiedKSatUpp)
                self.kUnsatLow = pcr.min(self.kUnsatLow, modifiedKSatLow)

                # kThVert (m.day-1) = unsaturated conductivity capped at field capacity
                # - exchange between layers capped at field capacity
                self.kThVertUppLow = pcr.sqrt(self.kUnsatUpp * self.kUnsatLow)
                # self.kThVertUppLow  = pcr.min(\
                #              pcr.sqrt(self.kUnsatUpp*self.kUnsatLow),\
    ...
    def estimateTranspirationAndBareSoilEvap(
        ...
        if self.numberOfLayers == 2 and returnTotalEstimation == False:
            actBareSoilEvap = (
                self.satAreaFrac
                * pcr.min(self.potBareSoilEvap, self.parameters.kSatUpp)
                + (1.0 - self.satAreaFrac)
                * pcr.min(self.potBareSoilEvap, self.kUnsatUpp)
