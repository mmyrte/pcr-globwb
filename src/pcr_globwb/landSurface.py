...
class LandSurface(object):
    def getState(self):
        ...
        if self.numberOfSoilLayers == 2:
            for coverType in self.coverTypes:
                ...
                result[coverType]["glacierIce"] = self.landCoverObj[
                    coverType
                ].glacierIce
                result[coverType]["glacierWater"] = self.landCoverObj[
                    coverType
                ].glacierWater
                # ADDED BY JOREN: STOP
    ...
    def __init__(self, iniItems, landmask, initialState=None):
        ...
        if "glacierModule" in list(self.iniItems.landSurfaceOptions.keys()):
            if self.iniItems.landSurfaceOptions["glacierModule"] == "True":
                self.glacierModule = True
            elif self.iniItems.landSurfaceOptions["glacierModule"] == "False":
    ...
    def obtainNonIrrWaterDemand(self, routing, currTimeStep):
        ...
        nonIrrigationWaterDemandDict["potential_demand"]["domestic"] = (
            self.domesticGrossDemand
        )
        nonIrrigationWaterDemandDict["potential_demand"]["industry"] = (
            self.industryGrossDemand
        )
        nonIrrigationWaterDemandDict["potential_demand"]["livestock"] = (
    ...
    def update(self, meteo, groundwater, routing, currTimeStep):
        ...
        if self.glacierModule == True:
            ...
            if (self.glacierType == "delta_H") & (currTimeStep.year > 1999):
                logger.info(
                    "DELTA H GLACIERS: Checks if we need to perform delta_H....."
                )
                ...
                if currTimeStep.doy == 245:
                    logger.info(
                        "DELTA H GLACIERS: Yes we do! Updating Delta H! (Huss et al., 2010; Seibert et al., 2018)........."
                    )
                    ...
                    for coverType in self.coverTypes:
                        ...
                        a, b, c = vos.getMinMaxMean(
                            pcr.spatial(self.landCoverObj[coverType].yearlyIceMelt)
                        )
                        msg = (
                            "BEFORE: Glacier yearlyIceMelt initialized: Min %f Max %f Mean %f"
                            % (a, b, c)
                        )
                        logger.info(msg)

                        a, b, c = vos.getMinMaxMean(
                            pcr.spatial(self.landCoverObj[coverType].yearlyGlacierAcc)
                        )
                        msg = (
                            "BEFORE: self.yearlyGlacierAcc initialized: Min %f Max %f Mean %f"
                            % (a, b, c)
                        )
                        logger.info(msg)
                    ...
                    logger.info(
                        "DELTA H GLACIERS: Finished with updating Delta H! (Huss et al., 2010; Seibert et al., 2018)........."
                    )

                    # Calculate glacier difference
                    self.glacierDifference = (
                        self.glacierIce - prevGlacierIce
                    ) * self.cellArea + (
                        self.glacierWater - prevGlacierWater
                    ) * self.cellArea

                    # Return glacier variables to different landCover types
                    for coverType in self.coverTypes:
                        logger.info(
                            "DELTA H GLACIERS: Returning glacier vars to: "
                            + str(coverType)
                        )
                        self.landCoverObj[
                            coverType
                        ].snowCoverSWE += self.glacierAccuCorrection
                        self.landCoverObj[coverType].yearlyIceMelt = self.yearlyIceMelt
                        self.landCoverObj[
                            coverType
                        ].yearlyGlacierAcc = self.yearlyGlacierAcc

                        a, b, c = vos.getMinMaxMean(pcr.spatial(self.yearlyIceMelt))
                        msg = (
                            "Glacier yearlyIceMelt initialized: Min %f Max %f Mean %f"
                            % (a, b, c)
                        )
                        logger.info(msg)
                        ...
                        msg = (
                            "self.yearlyGlacierAcc initialized: Min %f Max %f Mean %f"
                            % (a, b, c)
                        )
                        logger.info(msg)
                ...
                logger.info(
                    "IMMERZEEL GLACIERS: Sliding following Immerzeel et al., 2012....."
                )
        ...
        if self.glacierModule == True:
            if self.debugWaterBalance:
                if (self.glacierType == "delta_H") & (currTimeStep.year > 1999):
                    ...
                    self.glacierIncoming = (
                        self.glacierDifference
                    )  # +self.glacierAccuCorrection*self.cellArea
            ...
            vos.waterBalanceCheck(
                [self.glacierAccumulation, self.glacierIncoming / self.cellArea],
