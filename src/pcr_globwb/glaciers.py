...
def initializeGlacier(self, iniItems):
    ...
    self.glacierIce_ini = (
        self.glacierIce
    )  # Store initial glacier shapes for deltaH approach.
    ...
    if self.glacierType == "delta_H":
        ...
        if "ini_dH" in list(self.iniItems.landSurfaceOptions.keys()):
            if self.iniItems.landSurfaceOptions["ini_dH"] == "True":
                self.ini_dH = True
            elif self.iniItems.landSurfaceOptions["ini_dH"] == "False":
                self.ini_dH = False
        else:
            self.ini_dH = False
    ...
    if self.glacierType == "Immerzeel":
        ...
        self.cellLengthFD = (
            (self.cellArea / self.verticalSizeInMeter) ** (2)
            + (self.verticalSizeInMeter) ** (2)
        ) ** (0.5)
...
def updateStaticGlacier(self, meteo, currTimeStep):
    ...
    self.glacierAccumulation = pcr.ifthenelse(
        pcr.pcrand(self.glacierized > 0.0, self.snowCoverSWE > 0.0),
        self.AccEfficiency * self.snowCoverSWE,
        0,
    )  # Original HBV
    ...
    self.iceMelt = pcr.ifthenelse(
        pcr.pcror(
            (meteo.temperature <= self.freezingTGlacier), (self.snowCoverSWE > 0.0)
        ),
        0.0,
        pcr.min(
            self.glacierIce,
            self.glacierized
            * pcr.max(meteo.temperature - self.freezingTGlacier, 0.0)
            * (DDFGlacier),
        ),
    )
    ...
    self.snow2GlacierWater = (
        -pcr.min(0.0, self.deltaSnowCover) * self.glacierized
    )  # Convert to positive when added to glacier, remove refreezing

    self.rain2GlacierWater = pcr.ifthenelse(
        pcr.pcrand(self.glacierized > 0.0, self.snowCoverSWE == 0),
        self.liquidPrecip,
        0.0,
    )
    ...
    self.capacity2GlacierWater = pcr.ifthenelse(
        self.glacierized > 0.0,
        pcr.max(0.0, self.snowFreeWater - self.snowWaterHoldingCap * self.snowCoverSWE),
        0.0,
    )

    # update snowFreeWater (after capacity2GlacierWater)
    self.snowFreeWater = pcr.max(0.0, self.snowFreeWater - self.capacity2GlacierWater)
    ...
    self.glacierOutflow = pcr.min(
        self.glacierWater,
        self.glacierWater
        * (self.Kmin + self.Krange * pcr.exp(-1 * self.Ag * self.snowCoverSWE))
        * self.glacierized,
    )
    ...
    self.netGlacierWaterExcess = pcr.max(
        0.0, self.glacierWater - glacierWaterHoldingCap * self.glacierIce
    )
...
def glacierSlideImmerzeel(self, currTimeStep):
    ...
    u_two_fourth = pcr.max(
        0,
        (
            rho * g * self.glacierIce * pcr.sin(pcr.scalar(pcr.atan(self.gradient)))
            - tau_0
        )
        / (nu**2 * R),
    )  # [(m/s)**1/2]
    ...
    self.glacierOutgoing = pcr.ifthenelse(
        self.glacierIce > flowlim,
        self.glacierVelocity * (self.glacierIceVolume / self.channelLength),
        0,
    )
    self.glacierOutgoing = pcr.ifthenelse(
        self.glacierOutgoing > self.glacierIceVolume,
        self.glacierIceVolume,
        self.glacierOutgoing,
    )
    self.glacierIncoming = pcr.upstream(self.lddMap, self.glacierOutgoing)
    self.glacierIceVolume = pcr.max(
        0, self.glacierIceVolume + self.glacierIncoming - self.glacierOutgoing
    )
...
def updateDeltaH(self, currTimeStep):
    ...
    for i in range(101):
        shapes = pcr.readmap(
            self.glacierDir + "glacierShape_minus" + str(int(i)).zfill(2) + "M.map"
        )
    ...
    ratio = (
        glacierIceNew / Mnew
    )  # We need to divide it only over the area where there is now glacier.

    glacierGap = pcr.ifthenelse(
        glacierIceNew > 0, glacierGap, 0
    )  # We need to divide it only over the area where there is now glacier.
    glacierDiff = pcr.ifthenelse(
        pcr.pcrand(pcr.ordinal(frac) != pcr.ordinal(100), pcr.abs(glacierGap) > 1e-3),
        pcr.scalar(glacierGap * ratio),
        0.0,
    )  # Division based on ice thickness.
    ...
    positiveGap = pcr.ifthenelse(
        pcr.pcror(
            (frac < pcr.ordinal(0)),
            pcr.pcrand((frac == pcr.ordinal(0)), (glacierGap > 0)),
        ),
        glacierDiff,
        0.0,
    )  # Added pcr.ordinal(0) as a bug fix for multi-core processing.
    glacierDiff = pcr.ifthenelse(
        pcr.pcror(
            (frac < pcr.ordinal(0)),
            pcr.pcrand((frac == pcr.ordinal(0)), (glacierGap > 0)),
    ...
    transportFraction = pcr.ifthenelse(
        self.glacierIce > 0, pcr.scalar(1), pcr.scalar(0)
    )  # All ice is transported downwards
    added = pcr.accufractionstate(
        self.lddMap, positiveGap * self.cellArea, transportFraction
    )
...
def initializeDeltaH(self):
    ...
    for frac in fs_list:
        ...
        logger.info(
            ...
            + self.glacierDir
            + "glacierShape_minus"
            + str(int(frac)).zfill(2)
            + "M.map"
        )
        pcr.report(
            fs_map,
            self.glacierDir + "glacierShape_minus" + str(int(frac)).zfill(2) + "M.map",
        )
    ...
    deltaElev = (
        (pcr.roundoff((mx_elev + 50) / 100) - pcr.roundoff((mn_elev - 50) / 100))
        / steps
        * 100
    )  # Round to 100 and then divide into 20 steps. #Shouldn't divide into hundred steps, but into steps of 100....
    ...
    for k in range(len(fs_list)):
        ...
        if k != 0:
            ...
            iniMassDiff = (1 - fs_list[k - 1] / 100) * M - pcr.areatotal(
                volume, self.glacierNumber
            )
        ...
        glacierElev = pcr.ifthenelse(
            volume > 0, self.highResolutionDEM, 0
        )  # glacier elevation
        area_dH = {}
        Mnew = pcr.scalar(0.0)
        for i in range(steps):
            if i < (steps - 1):
                elev_range = pcr.pcrand(
                    glacierElev >= elev[i], glacierElev < elev[i + 1]
                )
            else:
                elev_range = pcr.pcrand(
                    glacierElev >= elev[i], glacierElev < elev[i] + 100
                )
            area_dH[i] = (
                pcr.areatotal(pcr.scalar(elev_range), self.glacierNumber) * dH[i]
            )  # Correct dH for the surface area of this elevation zone.
        ...
        for i in range(steps):
            ...
            temp_volume = pcr.ifthenelse(
                elev_range, temp_volume - fs * dH[i], temp_volume
            )  # Calculate the new glacier volume and distribution by applying the scaling factor to the deltaH function.
        ...
        pcr.report(
            fs_map,
            self.glacierDir + "glacierShape_minus" + str(int(k)).zfill(2) + "M.map",
        )
