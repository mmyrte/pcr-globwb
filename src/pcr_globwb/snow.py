...
def initializeSnow(self, iniItems):
    ...
    if "snowfallTransition" in list(self.iniItemsLC.keys()):
        if self.iniItemsLC["snowfallTransition"] == "True":
            self.snowfallTransition = True
        elif self.iniItemsLC["snowfallTransition"] == "False":
    ...
    if "exponentialMelt" in list(self.iniItemsLC.keys()):
        if self.iniItemsLC["exponentialMelt"] == "True":
            self.exponentialMelt = True
        elif self.iniItemsLC["exponentialMelt"] == "False":
    ...
    if self.snowTransport != False:
        ...
        self.reverseLDD = pcr.lddmask(
            self.reverseLDD, self.landmask
        )  # Added for transport issue
        ones = pcr.scalar(1.0)
...
def updateSnowFall(self, meteo, currTimeStep):
    ...
    if self.snowfallCorrection != False:
        ...
        self.estimSnowfall = pcr.ifthenelse(
            self.glacierIce > 0,
            self.estimSnowfall * self.snowfallCorrection,
            self.estimSnowfall,
        )
...
def snowMeltSlaterAndClark(self, meteo, currTimeStep):
    ...
    if self.albedoMelt == True:
        ...
        self.degreeDayFactor = (self.DDFmax - self.DDFmin) / (1 / 0.4 - 1 / 0.85) * (
            1 / self.albedo - 1 / 0.85
        ) + self.DDFmin
    else:
    ...
    if self.seasonalMelt == True:
        ...
        if self.exponentialMelt == False:
            ...
            deltaSnowCover = -pcr.min(
                self.snowCoverSWE + self.snowfall,
                pcr.max(
                    Pmelt
                    + Mm
                    * (ratio + pcr.ln(1 + pcr.exp(-1 * ratio)))
                    * pcr.max(
                        self.degreeDayFactor
                        + self.degreeDayAmplitude
                        * np.sin((currTimeStep.doy - 81) * 2 * np.pi / 366),
    ...
    self.snowFreeWater = pcr.max(0.0, self.snowFreeWater - self.actSnowFreeWaterEvap)
    # SCF_L[TYPE]= SCF_L[TYPE]-ES_a[TYPE]
    self.potBareSoilEvap = pcr.max(0, self.potBareSoilEvap - self.actSnowFreeWaterEvap)
    ...
    if self.debugWaterBalance:
        ...
        vos.waterBalanceCheck(
            [self.snowfall, self.liquidPrecip, self.incomingVolSnow / self.cellArea],
            [
